...
[1] 关于 Triple Zeta 这个说明，后文还会有具体的讨论。简单地说，这个词的使用在 Pople 提出的劈裂价层基组中，我认为也许少用为好。
...
[7] 关于 STO-3G 中 Primitive 系数的获得过程，其实现在已经不难做到了。在文档附带文件中，有一份 Mathematica 脚本，预计可以做这件事。但目前这份脚本尚未写完。

[8] Pople 文章中出现的 `J. Chem. Phys. 47, 201 (1967)` 是错的。其中的 `201` 应当是 `1201`。

[*] 关于这次文档的附带文件，可以到[下述文件夹](/assets/files/2017-12-20-Basis_One)找到。
...
6. 金杨阳提供的[材料](http://w3.iams.sinica.edu.tw/lab/wbtzeng/labtech/basis_set.htm)有也许比我更好的叙述（尽管网页可能难看了些）。
