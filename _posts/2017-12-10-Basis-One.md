...
根据上面对 2p 基组的叙述与讨论，我们应当很容易地写出 2p 基组的公式形式。注意到它与 2s 轨道的最大不同在于它有三种取向，反映在 $ (lmn) $ 的取值可以是 $ (100), (010), (001) $；这分别对应了 2p<sub>x</sub>, 2p<sub>y</sub>, 2p<sub>z</sub> 不同取向的轨道。我们下面以 $ (lmn) = (100) $ 为例，来分析 2p<sub>x</sub> 基组的公式形式：2p
\[
\begin{align*}
